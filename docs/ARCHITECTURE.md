...
- 统一入口与路由分发。
- JWT 校验，透传 `X-User-Id`、`X-Role` 与 `X-Permissions`。
...
- 网关统一 JWT 校验。
- 下游服务依赖 `X-User-Id`、`X-Role`、`X-Permissions` 执行角色与权限校验。

风险：

- 认证服务当前兼容历史明文密码并在登录时迁移为 BCrypt，仍需完成全量离线迁移与强策略校验。
- 管理员域接口当前仍以 `ADMIN` 角色边界为主，若引入子管理员需补充权限码级校验策略。
...
- 网关之外的服务级权限校验补齐（除 admin-service 外）[已完成，2026-03-04]。
